...
class ILoggingKind (ABC, IService):
	@abstractmethod
	def logData(self, *args, **kwargs) -> bool:
		raise NotImplementedError()

class Logger (IService): 
	...
	@property
	def loggingKind(self) -> list[ILoggingKind]:
		return _loggingKind
		
	@loggingKind.setter
	def loggingKind(self, kinds: list[ILoggingKind]) -> None: self._loggingKinds = kinds
...
class EmailLoggingKind (ILoggingKind):
	...
	def logData(self, *args, **kwargs) -> bool:
		try:
			...
			for i in args:
				content += f"{i}\n"
			return self._client.send(self._destination, self.MESSAGE_HEADER.format(datetime.now), content)
